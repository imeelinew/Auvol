...
github_repo="${AUVOL_GITHUB_REPOSITORY:-imeelinew/Auvol}"
team_id="${AUVOL_TEAM_ID:-5Q5QT76MJU}"
work_dir=$(mktemp -d /tmp/auvol-release.XXXXXX)
...
git add mac/Auvol.xcodeproj/project.pbxproj
if ! git diff --cached --quiet; then
    git commit -m "Release $tag"
fi

archive_path="$work_dir/Auvol.xcarchive"
derived_data="$work_dir/DerivedData"
xcodebuild archive \
    -project mac/Auvol.xcodeproj \
    -scheme Auvol \
...
app_path="$archive_path/Products/Applications/Auvol.app"
[[ -d "$app_path" ]] || {
...
archive_file="$work_dir/Auvol-${version}.zip"
